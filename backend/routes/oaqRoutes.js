...
router.get("/", async (req, res) => {
  try {
    ...
    if (sort === "votes") sortOpt = { netVotes: -1, createdAt: -1 };
    if (sort === "trending") sortOpt = { netVotes: -1, views: -1, createdAt: -1 };
...
/* ── Submit OAQ (with duplicate + out-of-scope detection) ── */
router.post("/", auth, async (req, res) => {
  try {
    ...
    const [faqDupes, oaqDupes] = await Promise.all([
      FAQ.find({
        $or: wordRegexes.flatMap((r) => [
          { "questions.q": r },
          { "questions.a": r },
        ]),
      }).lean(),
      OAQ.find({
        $or: wordRegexes.map((r) => ({ question: r })),
        status: { $ne: "rejected" },
      }).lean(),
    ]);
    ...
    const fuzzyTerms = words.map(
      (w) => new RegExp(w.split("").join(".*"), "i")
    );
    ...
    const allDupes = [
      ...
      ...oaqDupes
        ...
        .map((o) => ({
          ...
          source: "OAQ",
          id: o._id,
          score: score(o.question),
        })),
    ].sort((a, b) => b.score - a.score);

    const topDupes = allDupes.slice(0, 3);

    let outOfScope = false;
    if (allDupes.length === 0) {
      const allFaqTexts = faqDupes.flatMap((c) =>
        c.questions.map((item) => item.q + " " + item.a)
      );
      let bestFuzzy = 0;
...
router.post("/:id/answers", auth, async (req, res) => {
  try {
    ...
    if (oaq.status === "promoted" || oaq.status === "rejected") {
      return res
        .status(400)
        .json({ error: "Cannot answer a promoted or rejected question" });
    }
    ...
    if (oaq.submittedBy.toString() !== req.user._id.toString()) {
      await Notification.create({
        ...
        type: "answer",
        message: `${req.user.name} answered your question: "${oaq.question.slice(
          0,
          60
        )}${oaq.question.length > 60 ? "…" : ""}"`,
        link: "/community",
      });
    }
...
router.post("/:id/vote", auth, async (req, res) => {
  try {
    ...
    // Auto-promote if netVotes >= 10 and approved
    const netVotes =
      (oaq.votedUpBy || []).length - (oaq.votedDownBy || []).length;
    if (netVotes >= 10 && oaq.status === "approved") {
      const sortedAnswers = [...oaq.answers].sort(
        (a, b) =>
          (b.votedUpBy.length - b.votedDownBy.length) -
          (a.votedUpBy.length - a.votedDownBy.length)
      );
      const bestAnswer =
        sortedAnswers.find((a) => a.accepted) || sortedAnswers[0];
      ...
      await Notification.create({
        user: oaq.submittedBy,
        type: "promoted",
        message: `Your question was auto-promoted to FAQ: "${oaq.question.slice(
          0,
          60
        )}${oaq.question.length > 60 ? "…" : ""}"`,
        link: "/faq",
...
router.put("/:id/approve", auth, admin, async (req, res) => {
  try {
    ...
    await Notification.create({
      ...
      type: "approved",
      message: `Your question was approved: "${oaq.question.slice(
        0,
        60
      )}${oaq.question.length > 60 ? "…" : ""}"`,
      link: "/community",
...
router.put("/:id/reject", auth, admin, async (req, res) => {
  try {
    ...
    await Notification.create({
      user: oaq.submittedBy._id,
      type: "system",
      message: `Your question was rejected: "${oaq.question.slice(
        0,
        60
      )}${oaq.question.length > 60 ? "…" : ""}"`,
      link: "/community",
...
router.put("/:id/promote", auth, admin, async (req, res) => {
  try {
    const oaq = await OAQ.findById(req.params.id).populate(
      "submittedBy",
      "name"
    );
    if (!oaq) return res.status(404).json({ error: "Not found" });
    ...
    const bestAnswer = acceptedAnswer || sortedAnswers[0];
    const answerText = bestAnswer ? bestAnswer.text : oaq.question;
    ...
    await Notification.create({
      user: oaq.submittedBy._id,
      type: "promoted",
      message: `Your question was promoted to FAQ: "${oaq.question.slice(
        0,
        60
      )}${oaq.question.length > 60 ? "…" : ""}"`,
      link: "/faq",
  ...
  try {
    ...
    if (answer.accepted && answer.submittedBy) {
      await Notification.create({
        ...
        type: "accepted",
        message: `Your answer was accepted on: "${oaq.question.slice(
          0,
          60
        )}${oaq.question.length > 60 ? "…" : ""}"`,
        link: "/community",
