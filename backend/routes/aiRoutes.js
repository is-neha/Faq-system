...
const getGroq = () => new Groq({ apiKey: groqApiKey });
...
router.post("/summarize/:oaqId", auth, admin, async (req, res) => {
  try {
    if (!groqApiKey) {
      return res
        .status(400)
        .json({ error: "GROQ_API_KEY not configured. Add it to .env" });
    }

    const oaq = await OAQ.findById(req.params.oaqId).populate(
      "answers.submittedBy",
      "name"
    );
...
Respond in JSON format like this:
{
  ...
  "reason": "why this answer is best"
    ...
    res.json({
      ...
      bestAnswerIndex:
        result.bestAnswerIndex != null ? result.bestAnswerIndex - 1 : -1,
...
/* ── AI check if question is relevant to Vicharanashala topics ── */
